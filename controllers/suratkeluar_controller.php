...
//        foreach ($datas as $data) {
        ...
        $file = explode(".", $datas->getFile());
        $dir = '';
        if ($file[count($file) - 1] == 'pdf') {
            $dir = "arsip/";
        } else {
            $dir = "arsip/temp/";
        }
        $fp = fopen($dir . $datas->getFile(), 'r');
        $content = fread($fp, filesize($dir . $datas->getFile()));
        fclose($fp);
...
//        var_dump($datas);
     ...
     * bug jika telah diupload versi pdf-telah dianggap net, rekam revisi upload error
     */
...
//        }
        //---------------------------------
        $fln = array();
        if (file_exists('arsip/temp/' . $filename)) { //jika file telah ada di folder temp/
...
//                        var_dump($filename);
        ...
        if (($ext[count($ext) - 1]) == 'pdf') { //jika telah diupload tipe file pdf
            if (file_exists('arsip/' . $filename))
                copy('arsip/' . $filename, 'arsip/temp/' . $filename); //copykan ke folder temp/
        }else {
            $upload = new Upload('upload'); //jika tidak, upload file tersebut
